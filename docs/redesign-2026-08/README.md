...
- **Feedback round 1 (2026-08-22/23 night)** — Heinrich's walkthrough: explanatory micro-copy removed everywhere it had no functional job (explanations live in "How to read"); tiles spread their content (`Tile distribute`) instead of packing the top; blue + purple retired (`--accent-slate` = stone, `--accent-plum` = brown-red; palette = greens / warm reds-golds / neutrals); Dashboard hero pill border; Market recommendations as an in-place accordion (`?rec=`); Voice filter header rebuilt (selects in meta, wrapping tabs, audience pills in the tile when >3), category chips in green/red/gold/neutral families, short phrases only; Competitive face-off rows spread on one centre line; Content re-prioritised (what works leads 7×3, reply inbox 5×4 on the side). Commits `6c68f83 … 8ad5362` + the rec tweak. Rule added: `rounded-full` only on single-line pills.
