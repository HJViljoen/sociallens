...
async function main() {
  ...
  // on every video in arm B).
  const classificationChanges: Record<Bucket, { videos: number; changed: number; hookTextChanged: number; typeChanged: number }> = {
    client: { videos: 0, changed: 0, hookTextChanged: 0, typeChanged: 0 },
    competitor: { videos: 0, changed: 0, hookTextChanged: 0, typeChanged: 0 },
    industry: { videos: 0, changed: 0, hookTextChanged: 0, typeChanged: 0 },
  }
  for (const v of bed) {
    const b = bucketOf(v)
    ...
    if (typeChanged || hookChanged || a.hook_style !== bb.hook_style || a.sentiment !== bb.sentiment) classificationChanges[b].changed++
  }
  ...
  // status 'ok' must be zero across both arms.
  ...
  const junkLeaks = (leak ?? []).filter((r) => {
    const status = (r as unknown as { videos: { transcript_status: string | null } | null }).videos?.transcript_status
    return status !== 'ok'
  })
