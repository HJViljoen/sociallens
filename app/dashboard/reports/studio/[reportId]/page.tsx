...
export default async function StudioPage({ params }: { params: Promise<{ reportId: string }> }) {
  ...
  return (
    <PageFrame className="min-h-0 flex-1">
      <PageBar title={report.title} context={`Report Studio · ${sections.length} section${sections.length === 1 ? '' : 's'} · ${slideCount} slide${slideCount === 1 ? '' : 's'}`}>
        <Link href={`/dashboard/reports?group=reports&item=${report.id}`}><BarPill>Back to reports</BarPill></Link>
